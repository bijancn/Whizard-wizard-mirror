...
namespace Rivet {
  ...
  class WHIZARD_H4l2b_NLO : public Analysis {
...
#include "NLOHisto1D.cc"
  ...
  public:
    ...
    void init() {
      ...
      addProjection(mp, "mp");
      const IdentifiedFinalState hi(PID::HIGGS);
      addProjection(hi, "higgs");
      ...
      const double bin_low = 0.;
      const double bin_high = 800.;

      _h["leadingjet_E"] = bookNLOHisto1D("leading-jet-E", stdbin, bin_low, bin_high);
      _h["leadingjet_Pt"] = bookNLOHisto1D("leading-jet-pT", stdbin, bin_low, bin_high);
      _h["leadingjet_Theta"] = bookNLOHisto1D("leading-jet-Theta", stdbin, -1.1, 1.1);

      _h["2ndleadingjet_E"] = bookNLOHisto1D("2nd-leading-jet-E", stdbin, bin_low, bin_high);
      _h["2ndleadingjet_Pt"] = bookNLOHisto1D("2nd-leading-jet-pT", stdbin, bin_low, bin_high);
      _h["2ndleadingjet_Theta"] = bookNLOHisto1D("2nd-leading-jet-Theta", stdbin, -1.1, 1.1);

      _h["jets_invMass"] = bookNLOHisto1D("jets-inv", stdbin, bin_low, bin_high);
      ...
      _h["bjet_E"] = bookNLOHisto1D("b-jet-E", stdbin, bin_low, bin_high);
      _h["bjet_Pt"] = bookNLOHisto1D("b-jet-pT", stdbin, bin_low, bin_high);
      _h["bjet_Theta"] = bookNLOHisto1D("b-jet-Theta", stdbin, -1.1, 1.1);

      _h["bbarjet_E"] = bookNLOHisto1D("bbar-jet-E", stdbin, bin_low, bin_high);
      _h["bbarjet_Pt"] = bookNLOHisto1D("bbar-jet-pT", stdbin, bin_low, bin_high);
      _h["bbarjet_Theta"] = bookNLOHisto1D("bbar-jet-Theta", stdbin, -1.1, 1.1);

      _h["Wm_E"] = bookNLOHisto1D("Wm-E", stdbin, bin_low, bin_high);
      _h["Wm_Pt"] = bookNLOHisto1D("Wm-pT", stdbin, bin_low, bin_high);
      _h["Wm_Theta"] = bookNLOHisto1D("Wm-Theta", stdbin, -1.1, 1.1);
      _h["Wm_invMass"] = bookNLOHisto1D("Wm-inv", stdbin, bin_low, bin_high);
      _h["Wm_invMass_peak"] = bookNLOHisto1D("Wm-inv-peak", stdbin, 70., 90.);

      _h["Wp_E"] = bookNLOHisto1D("Wp-E", stdbin, bin_low, bin_high);
      _h["Wp_Pt"] = bookNLOHisto1D("Wp-pT", stdbin, bin_low, bin_high);
      _h["Wp_Theta"] = bookNLOHisto1D("Wp-Theta", stdbin, -1.1, 1.1);
      _h["Wp_invMass"] = bookNLOHisto1D("Wp-inv", stdbin, bin_low, bin_high);
      _h["Wp_invMass_peak"] = bookNLOHisto1D("Wp-inv-peak", stdbin, 70., 90.);

      _h["BWm_E"] = bookNLOHisto1D("BWm-E", stdbin, bin_low, bin_high);
      _h["BWm_Pt"] = bookNLOHisto1D("BWm-pT", stdbin, bin_low, bin_high);
      ...
      _h["BWm_invMass"] = bookNLOHisto1D("BWm-inv", stdbin, bin_low, bin_high);
      _h["BWm_invMass_peak"] = bookNLOHisto1D("BWm-inv-peak", stdbin, 160., 180.);

      _h["BWp_E"] = bookNLOHisto1D("BWp-E", stdbin, bin_low, bin_high);
      _h["BWp_Pt"] = bookNLOHisto1D("BWp-pT", stdbin, bin_low, bin_high);
      ...
      _h["BWp_invMass"] = bookNLOHisto1D("BWp-inv", stdbin, bin_low, bin_high);
      _h["BWp_invMass_peak"] = bookNLOHisto1D("BWp-inv-peak", stdbin, 160., 180.);

      _h["Blm_E"] = bookNLOHisto1D("Blm-E", stdbin, bin_low, bin_high);
      _h["Blm_Theta"] = bookNLOHisto1D("Blm-Theta", stdbin, -1.1, 1.1);
      _h["Blm_Pt"] = bookNLOHisto1D("Blm-pT", stdbin, bin_low, bin_high);
      _h["Blm_invMass"] = bookNLOHisto1D("Blm-inv", stdbin, bin_low, bin_high);
      _h["Blm_Phi"] = bookNLOHisto1D("Phi(b,lm)", stdbin, 0., M_PI);
      _h["Blm_R"] = bookNLOHisto1D("R(b,lm)", stdbin, 0., 5.);

      _h["Blp_E"] = bookNLOHisto1D("Blp-E", stdbin, bin_low, bin_high);
      _h["Blp_Theta"] = bookNLOHisto1D("Blp-Theta", stdbin, -1.1, 1.1);
      _h["Blp_Pt"] = bookNLOHisto1D("Blp-pT", stdbin, bin_low, bin_high);
      _h["Blp_invMass"] = bookNLOHisto1D("Blp-inv", stdbin, bin_low, bin_high);
      _h["Blp_Phi"] = bookNLOHisto1D("Phi(b,lp)", stdbin, 0, M_PI);
      _h["Blp_R"] = bookNLOHisto1D("R(b,lp)", stdbin, 0., 5.);

      _h["BB_E"] = bookNLOHisto1D("BB-E", stdbin, bin_low, bin_high);
      _h["BB_Theta"] = bookNLOHisto1D("BB-Theta", stdbin, -1.1, 1.1);
      _h["BB_Pt"] = bookNLOHisto1D("BB-pT", stdbin, bin_low, bin_high);
      _h["BB_invMass"] = bookNLOHisto1D("BB-inv", stdbin, bin_low, bin_high);
      ...
      _h["W_invMass"] = bookNLOHisto1D("W-inv", stdbin, bin_low, bin_high);
      _h["W_invMass_peak"] = bookNLOHisto1D("W-inv-peak", stdbin, 70., 90.);
      _h["BW_invMass"] = bookNLOHisto1D("BW-inv", stdbin, bin_low, bin_high);
      _h["BW_invMass_peak"] = bookNLOHisto1D("BW-inv-peak", stdbin, 160., 180.);
      _h["Bl_invMass"] = bookNLOHisto1D("Bl-inv", stdbin, bin_low, bin_high);

      _h["hardest_lepton_E"] = bookNLOHisto1D("hardest-lepton-E", stdbin, bin_low, bin_high);
      _h["hardest_lepton_Pt"] = bookNLOHisto1D("hardest-lepton-pT", stdbin, bin_low, bin_high);
      _h["hardest_lepton_Eta"] = bookNLOHisto1D("hardest-lepton-eta", stdbin, -3., 3.);

      _h["2nd_hardest_lepton_E"] = bookNLOHisto1D("2nd-hardest-lepton-E", stdbin, bin_low, bin_high);
      _h["2nd_hardest_lepton_Pt"] = bookNLOHisto1D("2nd-hardest-lepton-pT", stdbin, bin_low, bin_high);
      _h["2nd_hardest_lepton_Eta"] = bookNLOHisto1D("2nd-hardest-lepton-Eta", stdbin, -3., 3.);

      _h["electron_E"] = bookNLOHisto1D("electron-E", stdbin, bin_low, bin_high);
      _h["electron_Pt"] = bookNLOHisto1D("electron-Pt", stdbin, bin_low, bin_high);
      _h["electron_Eta"] = bookNLOHisto1D("electron-Eta", stdbin, -3., 3.);

      _h["muon_E"] = bookNLOHisto1D("muon-E", stdbin, bin_low, bin_high);
      _h["muon_Pt"] = bookNLOHisto1D("muon-Pt", stdbin, bin_low, bin_high);
      ...
      _h["lepton_lepton_E"] = bookNLOHisto1D("lepton-lepton-E", stdbin, bin_low, bin_high);
      _h["lepton_lepton_Pt"] = bookNLOHisto1D("lepton-lepton-Pt", stdbin, bin_low, bin_high);
      _h["lepton_lepton_Theta"] = bookNLOHisto1D("lepton-lepton-Theta", stdbin, -1.1, 1.1);

      _h["MET"] = bookNLOHisto1D("MET", stdbin, bin_low, bin_high);
      _h["MPT"] = bookNLOHisto1D("MPT", stdbin, bin_low, bin_high);
      ...
      _h["Higgs_E"] = bookNLOHisto1D("Higgs-E", stdbin, bin_low, bin_high);
      _h["Higgs_Pt"] = bookNLOHisto1D("Higgs-Pt", stdbin, bin_low, bin_high);
      _h["Higgs_Theta"] = bookNLOHisto1D("Higgs-Theta", stdbin, -1.1, 1.1);
      _h["Higgs_leadingjet_R"] = bookNLOHisto1D("Higgs-leading-jet-R", stdbin, 0., 5.);
      _h["Higgs_secondjet_R"] = bookNLOHisto1D("Higgs-2nd-leading-jet-R", stdbin, 0., 5.);

      _h["BWpBWm_inv"] = bookNLOHisto1D("BWpBWp-inv", stdbin, bin_low, bin_high);
      _h["BWpBWm_E"]   = bookNLOHisto1D("BWpBWm-E", stdbin, bin_low, bin_high);
      _h["BWpBWm_pT"]  = bookNLOHisto1D("BWpBWm-pT", stdbin, bin_low, bin_high);
      _h["BlpBlm_inv"] = bookNLOHisto1D("BlpBlp-inv", stdbin, bin_low, bin_high);
      _h["BlpBlm_E"]   = bookNLOHisto1D("BlpBlm-E", stdbin, bin_low, bin_high);
      _h["BlpBlm_pT"]  = bookNLOHisto1D("BlpBlm-pT", stdbin, bin_low, bin_high);

      _h["BWpH_inv"] = bookNLOHisto1D("BWpH-inv", stdbin, bin_low, bin_high);
      _h["BWpH_E"]   = bookNLOHisto1D("BWpH-E", stdbin, bin_low, bin_high);
      _h["BWpH_pT"]  = bookNLOHisto1D("BWpH-pT", stdbin, bin_low, bin_high);
      _h["BlpH_inv"] = bookNLOHisto1D("BlpH-inv", stdbin, bin_low, bin_high);
      _h["BlpH_E"]   = bookNLOHisto1D("BlpH-E", stdbin, bin_low, bin_high);
      _h["BlpH_pT"]  = bookNLOHisto1D("BlpH-pT", stdbin, bin_low, bin_high);
    ...
    void analyze(const Event& event) {
      ...
      bool register_bwm = lm.size() > 0 && nb.size() > 0 && bbarjets.size() > 0;
      bool register_bwp = lp.size() > 0 && n.size() > 0 && bjets.size() > 0;
      FourMomentum Topbar;
      if(register_bwm) {
      ...
      FourMomentum TopTotal;
      if(register_bwm && register_bwp) {
        TopTotal = Top + Topbar;
        _h["BWpBWm_inv"]->fill(TopTotal.mass(), event);
        _h["BWpBWm_E"]->fill(TopTotal.E(), event);
        _h["BWpBWm_pT"]->fill(TopTotal.pt(), event);
      }

      bool register_blm = lm.size() > 0 && bbarjets.size() > 0;
      bool register_blp = lp.size() > 0 && bjets.size() > 0;
      FourMomentum Blm;
      if(register_blm) {
      ...
      FourMomentum BlTotal;
      if (register_blm && register_blp) {
         BlTotal = Blm + Blp;
         _h["BlpBlm_inv"]->fill(BlTotal.mass(), event);
         _h["BlpBlm_E"]->fill(BlTotal.E(), event);
         _h["BlpBlm_pT"]->fill(BlTotal.pt(), event);
      }  
      ...
      _h["Higgs_leadingjet_R"]->fill(Rivet::deltaR(jets[0].momentum(),H[0].momentum()), event);
      _h["Higgs_secondjet_R"]->fill(Rivet::deltaR(jets[1].momentum(),H[0].momentum()), event);

      FourMomentum BWpH;
      if (register_bwp) {
        BWpH = Top + H[0].momentum();
        _h["BWpH_inv"]->fill(BWpH.mass(), event);
        _h["BWpH_E"]->fill(BWpH.E(), event);
        _h["BWpH_pT"]->fill(BWpH.pt(), event);
      }
      
      FourMomentum BlpH;
      if (register_blp) {
        BlpH = Blp + H[0].momentum();
        _h["BlpH_inv"]->fill(BlpH.mass(), event);
        _h["BlpH_E"]->fill(BlpH.E(), event);
        _h["BlpH_pT"]->fill(BlpH.pt(), event);
      }
    } // analyze
  ...
  DECLARE_RIVET_PLUGIN(WHIZARD_H4l2b_NLO);
