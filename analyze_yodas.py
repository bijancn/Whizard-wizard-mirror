...
parser.add_argument('--search-negative-histograms', dest='negative_yodas', type=bool, default=False)
...
def check_yoda_for_negative_histogram (filename):
  ...
  for line in searchfile:
    if "Area" in line:
      elements = line.split()
      if float(elements[2]) < 0.0: negative_histos.append (histo_nr)
      histo_nr += 1
...
class yoda_histo (object):
  ...
  def __str__(self):
    return "n_bins: {0}\n mean: {1}\n variance: {2}\n deviating yodas{3}: ".format(self.n_bins,self.mean,self.variance,self.deviant)
    ...
    if current_name in yoda_histos:
      if read_data and i_bin < yoda_histos[current_name].n_bins:
        ...
        if n_entries > 0:
          ...
          yoda_histos[current_name].mean[i_bin] = \
            compute_elemental_mean (yoda_histos[current_name].mean[i_bin], float(x[2]), n_yoda)
          yoda_histos[current_name].variance[i_bin] = \
            compute_elemental_variance (yoda_histos[current_name].variance[i_bin], \
            old_mean, yoda_histos[current_name].mean[i_bin], n_yoda)
        i_bin += 1
      else:
        read_data = False
        i_bin = 0
    if not read_data: read_data = is_data_beginning (line)
  searchfile.close()
    ...
    if current_name in yoda_histos:
      if read_data and i_bin < yoda_histos[current_name].n_bins:
        ...
        if (abs (current_mean) > 0.0 and current_sigma > 0.0):
          ...
          if pull > tolerance and not i_yoda in yoda_histos[current_name].deviant:
...
def create_histos (base_name, i_yodas):
  ...
  for line in searchfile:
    ...
    if count_bins and not is_histogram_end (line):
      ...
      n_bins = 0
      count_bins = False
    if not count_bins: count_bins = is_data_beginning (line)
  searchfile.close()
  return yoda_histos
...
def loop_over_all_yodas_and_compute_mean (base_name, i_yodas, yoda_histos):
  ...
  for i_yoda in i_yodas:
    yoda_name = base_name + '-' + str(i_yoda)
    compute_mean (yoda_name, yoda_histos, n_yodas)
    n_yodas += 1
  return yoda_histos
...
if args.verbose: print_all_histos (yoda_histos)
count_deviators = np.zeros (n_yodas_max)
for name, yh in yoda_histos.items():
  for deviator in yh.deviant:
    count_deviators[deviator] += 1

if args.verbose: print 'How often does a yoda deviate? ', count_deviators
